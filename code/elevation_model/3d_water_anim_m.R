...
for (i in 1:nrow(elevation.df)) {
  print(paste0("Rendering frame ", i, " of ", nrow(elevation.df), "."))
  
  render_water(
    elevation_matrix_ft,
    waterdepth = elevation.df[i, 'elev_ft'],
    watercolor = "#0F2419",
    wateralpha = 1,
    zscale = 15
  )
  render_snapshot(
    filename = sprintf("imgs_m/gsl%i.png", i),
    title_text = paste(
      "Great Salt Lake Change in Lakebed Exposure |",
      # month(elevation.df[i, 'datetime'], label = TRUE),
...
system(
  "ffmpeg -framerate 30 -i ./imgs_m/gsl%d.png -pix_fmt yuv420p vid_m/gsl.mp4"
)
